...
const Comments = require('lib/backend/api-v2/db-api/comments')
...
exports.findByDate = function findByDate (options, fn) {
  if (typeof options === 'function') {
    fn = options
    options = undefined
  }

  log(`Get topics order by the newest.`)

  let query = Topic
    .find({ deletedAt: null })
    .sort({ publishedAt: -1 })
    .populate('tag participants forum')
...
exports.findByClosed = function findByClosed (options, fn) {
  ...
  let query = Topic
    .find({ deletedAt: null, 'closingAt': { $lte: new Date() } })
    .populate('tag participants forum')
...
exports.findByPopular = function findByPopular (options, fn) {
  ...
  query
    .exec(function (err, topics) {
      Promise.all(topics.map((topic) => {
        ...
        return Comments.commentersCount({ topicId: topic.id })
          .then((commentersCount) => {
            console.log(topic.id, commentersCount)
            topic.commentersCount = commentersCount
            return topic
          })
        }))
  ...
  // query
  //   .exec(function (err, topics) {
  //     return fn(err, topics.sort(sortByCommentsCount).reverse())
  //     // Promise.all(topics.map((forum) => {
  //       // forum = forum.toJSON()
  //       // return Topic
